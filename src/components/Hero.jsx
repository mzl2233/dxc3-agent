...
const LINES = [
  ...
  { t: 2400, c: 'text-yellow-300', s: '[MIMO] 识别到 12 个可执行动作' },
  ...
  { t: 7500, c: 'text-yellow-300', s: '✅ 全部日常任务完成 · 耗时 ~85s' },
]
  ...
  return (
    <section className="min-h-screen flex flex-col items-center justify-center px-4 pt-16 pb-8">
      <div className="relative max-w-4xl mx-auto w-full text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-violet-700/40 bg-violet-950/30 text-violet-300 text-xs mb-6">
          <Zap size={12} className="text-yellow-400" />
          <span>MIMO 模型驱动 · ADB 自动化 · B站攻略解析</span>
        </div>

        <h1 className="text-5xl sm:text-7xl font-black mb-3">
          <span className="gold">地下城堡3</span>
        </h1>
        <p className="text-2xl font-bold text-slate-300 mb-4">AI Agent 自动化</p>
        <p className="text-slate-400 text-lg max-w-xl mx-auto mb-10 leading-relaxed">
          从 B 站维基爬取攻略 → 调用 <span className="text-yellow-400 font-mono">mimo-v2.5-pro</span> 解析成动作序列<br />
          → ADB 控制雷电模拟器执行 → OCR 验证结果，全程无人工介入
        </p>

        <div className="max-w-2xl mx-auto mb-10">
          <div className="term gold-glow">
            <div className="term-bar">
              <div className="dot bg-red-500" /><div className="dot bg-yellow-500" /><div className="dot bg-green-500" />
              <span className="ml-3 text-slate-500 text-xs font-mono">python main.py --task auto</span>
            </div>
            <div className="p-4 sm:p-5 min-h-[260px] space-y-1.5">
              {LINES.slice(0, shown).map((l, i) => (
                <p key={i} className={`font-mono text-xs sm:text-sm up ${l.c}`}>{l.s}</p>
              ))}
              {shown < LINES.length && <span className="font-mono text-xs text-yellow-400 blink">█</span>}
            </div>
          </div>
        </div>

        <a href="https://github.com/mzl2233/dxc3-agent" target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-semibold rounded-xl transition-all hover:scale-105 text-sm">
          <Github size={16} /> 查看源码
        </a>
      </div>
    </section>
